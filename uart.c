...
void putch(unsigned char byte) 
{
	...
	while(!TXIF)    //blocking delay because until the completion of transmission then only we can proceed with the next instruction
	{
		continue;
	}
	TXIF = 0;
...
unsigned char getch(void)
{
	...
	while(!RCIF)   //only one byte can receive
	{
		continue;
	}
	RCIF = 0;
	return RCREG;   //receiver register
}

unsigned char getche(void)   //transmit and receive data simultaneously
{
	unsigned char c;
